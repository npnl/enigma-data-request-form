...
const UserDataInputs: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (isSubmitEnabled) {
      ...
      Object.keys(metricsData.behavioralMetrics).flatMap(category =>
        Object.values(metricsData.behavioralMetrics[category]).filter((metric) => metric.is_selected).map((metric: Metric) => {
      ...
      Object.values(metricsData.imagingMetrics['imaging']).filter((metric: Metric) => metric.is_selected).map((metric: Metric) => {
          let metricData: MetricOut = {
            required: metric.is_required,
            value1: metric.filter_value1 || "",
            value2: metric.filter_value2 || "",
        ...
        if (Object.keys(data.imaging).length == 0 && Object.keys(data.behavior).length == 0) {
            dispatch(showModal({ title: "Warning", message: "You haven't selected any data!", modalType: null }));
        } 
        else {

        try {
            const response = await ApiUtils.submitRequest(data);
            dispatch(showModal({ title: "Success", message: "Your request has been successfully submitted!", modalType: 'success' }));
          } catch (error) {
  ...
  return (
    <>
    <label className="mb-1 mt-2">If you have any additional notes for us, please provide them in the box below</label>
    <Form.Control as="textarea" rows={4} placeholder="Notes" onChange={(e) => setNotes(e.target.value)}/>
    <div className="mt-4 mb-5">
      ...
      <Button variant="primary" onClick={handleSubmit} disabled={!isSubmitEnabled}>
        Submit Request
      </Button>
    </div>
    </>
