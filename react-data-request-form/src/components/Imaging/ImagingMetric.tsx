...
const ImagingMetricInput: React.FC<ImagingMetricProps> = ({
  ...
  const metricData = useSelector((state: RootState) => state.metrics.imagingMetrics[category].find(metric => metric.metric_name === metricName)) as Metric;
  ...
  return (
    ...
    <div className="form-check">
        ...
        <label className="form-check-label text-break" htmlFor={metricName}>
          {Object.entries(imageMetricMapping).find(([key, value]) => key === metricName)?.[1] ?? ''}
        </label>
      </div>
