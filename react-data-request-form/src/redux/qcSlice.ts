...
const qcDataSlice = createSlice({
    ...
    reducers: {
        ...
        setErrors(state, action: PayloadAction<Dictionary<string | number> | null>) {
        ...
        resetDataChanged(state) {
            ...
            state.initialData = state.data ? JSON.parse(JSON.stringify(state.data)) : null;
        },
    },
});

// Export actions
export const { setBidsId, setSesId, setData, setErrors, updateDataKey, resetDataChanged } =
    qcDataSlice.actions;
